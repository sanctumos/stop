...
# source must never contain destroy verbs targeting the session (ignore comments)
if grep -vE '^[[:space:]]*#' "$ROOT/tools/reload-in-screen.sh" \
  | grep -EEq -- '-X[[:space:]]+(quit|kill)|screen[[:space:]]+-wipe|killall[[:space:]]+screen'
then
